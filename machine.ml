...
let rec pp_term =
 ...
 | Proj(i,t) -> "P" ^ string_of_int i ^ pp_term t
...
let pp_stack_item =
 ...
 | SClos(f,n,t) ->
    "[" ^ string_of_int f ^ "," ^ string_of_int n ^ "," ^ pp_term t ^ "]"
...
let rec run : status -> value = function status ->
 prerr_endline (pp_status status ^ "\n->\n") ;
 match status with
 ...
 | (false,Clos(f,n,t,us),k,e,a) ->(*sea6*)
    run (false,Tuple us,SClos(f,n,t)::k,e,a)
 | (false,L n,k,((fs,_) as e),a) when n < Array.length fs ->(*sub1*)
     run (true,fs.(n),k,e,a)
 | (false,S n,k,((_,ns) as e),a) when n < Array.length ns ->(*sub2*)
     run (true,ns.(n),k,e,a)
 ...
 | (true,Tuple vs,SProj i::k,e,a) when i < Array.length vs ->(*pi*)
    run (true,vs.(i),k,e,a)
 | (true,Tuple vs,SClos(f,n,t)::k,e,a) ->(*sea7'*)
    run (true,Clos(f,n,t,vs),k,e,a)
 | (true,v,[],_,(k,e)::a) ->(*sea7'*)
    run (true,v,k,e,a)
